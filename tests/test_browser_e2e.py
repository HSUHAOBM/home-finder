    ...
    with isolated_app(tmp_path) as app:
        ...
        try:
            with sync_playwright() as playwright:
                ...
                reference = workspace.locator(".real-price-listing-reference")
                expect(reference).to_be_visible()
                expect(reference).to_contain_text("目前收藏房源")
                expect(reference).to_contain_text("1,150 萬")
                expect(reference).to_contain_text("權狀坪數")
                expect(workspace.locator(".similarity-help")).to_contain_text("可能為本收藏房")
