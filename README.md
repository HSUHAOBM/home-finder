...
目前正式入口是 `home_finder.web_app_v17`，主要功能包括：

- 大樓／華廈、透天／車墅、預售屋分開評估；公寓不蒐集。
- 預售屋整合 591 與樂居新建案清單；樂居用於補足 591 未呈現的建案，同名同區建案會在地圖合併來源連結。
- 預售屋提供「地圖探索」，不因總價、主建物、格局或戶別車位尚未公開而隱藏建案；先以行政區約略位置顯示供給，再依公開資料完整度篩選。
- 透天／車墅可再用「透天」與「別墅」標籤快速篩選；若廣告雖標示透天或別墅，但樓層結構明顯屬於集合住宅，或詳情明確寫電梯大樓／華廈，會直接列入透天的「已排除」。
...
- 同區、同價、主建坪數與格局近乎相同且標題高度相似的重複刊登，即使其中一筆缺地址仍會合併；搜尋任一原始編號都能找到合併卡。
...
```powershell
uv run python -m home_finder.web_app_v17
```
...
1. 選擇購屋目標。
2. 選擇「每日更新」、「近 7 天／每區 5 頁」、「近 10 天／每區 7 頁」、「近 15 天／每區 15 頁」或「完整盤點」。
...
9. 選擇「預售屋」後按「地圖探索」，先查看各行政區有哪些建案；圖釘為行政區約略位置，不代表基地座標。
